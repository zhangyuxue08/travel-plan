...
function mapLabelConnectorEnd(place, labelLayout = place) {
  const tx = Number(labelLayout.x ?? place.tx);
  const ty = Number(labelLayout.y ?? place.ty);
  const anchor = labelLayout.anchor || place.anchor || "start";
  const lineCount = Array.isArray(place.lines) && place.lines.length > 1 ? place.lines.length : 1;
  return {
    x: anchor === "end" ? tx + 8 : tx - 8,
    y: ty + (lineCount > 1 ? 10 : -8)
  };
}

function mapLabelConnectorStart(place, end) {
  const dx = end.x - place.x;
  const dy = end.y - place.y;
  const distance = Math.max(1, Math.hypot(dx, dy));
  const radius = 14;
  return {
    x: place.x + dx / distance * radius,
    y: place.y + dy / distance * radius
  };
}
...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  <g id="overview-label-connectors" inkscape:groupmode="layer" inkscape:label="标注引线" fill="none" stroke="#092653" stroke-width="1.4" stroke-linecap="round" stroke-opacity=".42">${placeLayers.map((place) => { const end = mapLabelConnectorEnd(place); const start = mapLabelConnectorStart(place, end); return `<line id="overview-label-stem-${esc(place.id)}" x1="${start.x.toFixed(1)}" y1="${start.y.toFixed(1)}" x2="${end.x.toFixed(1)}" y2="${end.y.toFixed(1)}"/>`; }).join("")}</g>
