...
function mapArtwork(source, selected, id, viewport) {
  ...
  svg.querySelectorAll('[id^="overview-label-stem-"]').forEach((stem) => {
    const placeId = stem.id.replace("overview-label-stem-", "");
    if (!layout?.places.includes(placeId)) { stem.remove(); return; }
    const place = placeLayersFor(source).find((item) => item.id === placeId);
    const labelLayout = layout.labels?.[placeId] || { x: place?.tx, y: place?.ty, anchor: place?.anchor };
    if (!place || !Number.isFinite(Number(labelLayout.x)) || !Number.isFinite(Number(labelLayout.y))) { stem.remove(); return; }
    const end = mapLabelConnectorEnd(place, labelLayout);
    const start = mapLabelConnectorStart(place, end);
    stem.setAttribute("x1", start.x.toFixed(1));
    stem.setAttribute("y1", start.y.toFixed(1));
    stem.setAttribute("x2", end.x.toFixed(1));
    stem.setAttribute("y2", end.y.toFixed(1));
  });
